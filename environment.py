...
from mininet.node import Controller, OVSKernelSwitch # ALTERAÇÃO: Importado OVSController
...
def setup_environment(NUM_HOSTS, TRAFFIC_DURATION):
    ...
    net = Mininet(switch=OVSKernelSwitch, controller=Controller, link=TCLink, cleanup=True) #
    
    info('*** Adicionando controlador\n')
    net.addController('c0')
    
    info('*** Adicionando switch e hosts\n')
    switch = net.addSwitch('s1', protocols='OpenFlow10', stp=True) # Usar STP pode ajudar em topologias mais complexas
    ...
    for i in range(1, NUM_HOSTS + 1):
        ...
        net.addLink(host, switch, bw=BW, delay=DELAY, loss=LOSS, max_queue_size=MAX_QUEUE_SIZE, r2q=1)
        hosts.append(host)
    
    receiver = net.addHost('h0')
    net.addLink(receiver, switch, bw=BW, delay=DELAY, loss=LOSS, max_queue_size=MAX_QUEUE_SIZE, r2q=1)
...
def start_traffic(net, hosts, receiver, TRAFFIC_DURATION, results_dir_abs, algorithm):
    ...
    for host in hosts:
        ...
        #host.cmd(f'iperf -c {receiver_ip} -n 1M > /dev/null 2>&1 &')
...
def configure_network_post_start(net, algorithm, hosts, receiver):
    ...
    for host in hosts:
     ...
     # --- CORREÇÃO FINAL: Configurar também o Host Receptor ---
    if algorithm == 'dctcp':
        info("--> Configurando ECN no receptor h0...\n")
        receiver.cmd('sysctl -w net.ipv4.tcp_ecn=1')
    # --- FIM DA CORREÇÃO ---
    ...
    if algorithm == 'dctcp':
        ...
        # --- CORREÇÃO: Diminua drasticamente o kthresh ---
        # Em vez de 50% do buffer, vamos usar um valor fixo e baixo.
        # Para um buffer de 200, 20-30 pacotes (10-15%) é um bom ponto de partida.
        kthresh = str(30) # <<< VALOR BAIXO E SENSÍVEL
